...
def data_collator(batch, llm_tokenizer, feature_extractor):
    ...
    # Clean and process output texts
    texts_cleaned = [clean_text(text) for text in output_texts]
    tokenized_texts = [tokenize_text(text, llm_tokenizer) for text in texts_cleaned]

    # Process labels
    max_text_length = max(len(tokenized_text['input_ids'][0]) for tokenized_text in tokenized_texts)
    labels_ids = []
    labels_masks = []
...
class SLAM_ASR(l.LightningModule):
    def __init__(self, batch_size: int = 1, warmup_steps: int = 1000, lr: float = 1e-4):
        ...
        self.assistant_prompt_embeds, self.assistant_mask = get_text_embedding(
            "<|eot_id|><|start_header_id|>assistant<|end_header_id|>",  self.llm_model, self.llm_tokenizer, return_attention_mask=True)
    ...
    def training_step(self, batch, batch_idx):
        ...
        start_idx = user_prompt_embeds.shape[1] + projected_embeds.shape[1] + assistant_prompt_embeds.shape[1]
        labels[:, start_idx:start_idx + labels_ids.shape[1]] = torch.where( # 从start_idx开始计算loss之前的用-100填充保持对其 loss不会被prefix影响
